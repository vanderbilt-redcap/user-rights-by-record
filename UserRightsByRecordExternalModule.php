...
class UserRightsByRecordExternalModule extends AbstractExternalModule
{
	...
	private function getFormAccess($project_id,$role_id) {
		...
		$sql = "SELECT data_entry
			...
			AND role_id=?";
		//echo "$sql<br/>";
		$roleForms = db_result($this->query($sql,[$this->escape($project_id),$this->escape($role_id)]),0);
	...
	public function getAutoId($project_id) {
		...
		$sql = "SELECT MAX(CAST(record as UNSIGNED))
				...
				WHERE project_id=?";
		//echo "$sql<br/>";
		$highestRecord = db_result($this->query($sql,[$this->escape($project_id)]),0);
		$highestRecord++;
	...
	public function getFormStatus($project_id,$record_id) {
		...
		//echo "$sql<br/>";
		$result = $this->query($sql,[$this->escape($project_id),$this->escape($record_id)]);
	...
	function getUserList($project_id) {
		...
		WHERE d.project_id=?";
		$result = $this->query($sql,[$this->escape($project_id)]);
		while ($row = db_fetch_assoc($result)) {
